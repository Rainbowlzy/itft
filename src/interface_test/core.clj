...
(def member-id-order ""  (decrypt-with-hello "h5kojvlkzO+9dd6HDW2MhqTWYwachDSX5H1yrN+iqdB381Uv9jtG8b1VosaFyH63"))
(def current-version "" (decrypt-with-hello "9wzxSGWFFBF9WyUDK9NQpA=="))
...
(def config  "Global configuration entity."  (load-file (decrypt-with-hello "/DK8F/+EHZcNv5MZLP/+n1PNHi7TTbiv06rguTyMPdmnnEKnmUErX2+HibXTl2peokLxDfrJKClaB703O/W4wl5VkDpEF6Ej1fETrfAYrbO1LHLerVt0HwUVcXml9UUu")))
  ...
  ([params service client version url deviceid]
   (let [request {:request
                  {:body
                   (into {:clientInfo
                          {:deviceId deviceid,
                           :extend (str (decrypt-with-hello "6/JhO8cX4mPtUVTvBLaGMwRjA0KGDucAVvk8fUzXIMQ=") client (decrypt-with-hello "jzMwTvclfszpEC8N2KGulw==") client "6_2"),
                           :mac (decrypt-with-hello "94ldCBnqQeNttJAkjqZP6FoOSOjQpPw3KlvC7OYfrOwEYwNChg7nAFb5PH1M1yDE"),
                           :refId (decrypt-with-hello "W7JLTEotMrh8k7jiryajbg=="),
                           :versionType (str client),
                           :clientIp (decrypt-with-hello "2EVz8WAdpjmZ5nhyDR8DYg=="),
                           :networkType "wifi",
                           :versionNumber (clojure.string/join "." (filter #(not (empty? %)) (clojure.string/split (str version) #"")))}}
                         params),
                   :header {:accountID (decrypt-with-hello "cQJNrmwGSj4ooOQokcHxmxUpuAnO6yfjyTbh1VagTB7SbbYVZTSmRZhYBAI7Sd8X"),
                            :encryptEffort "0",
                            :serviceName (str service),
                            :reqTime (decrypt-with-hello "znlOCEZFLcZqbhUebg6luQ=="),
                            :version (decrypt-with-hello "F1kKdUsWv1bJ/Ogyy9D40w==")}}}
...
(defn call-default
  ([sv] (call-default sv {}))
  ([sv params] (call-service params sv (decrypt-with-hello "+RidXrwPi5LFSpJthYo6Ng==") (decrypt-with-hello "lPFf+SWe/KEtMa9Q7ZMhvg==") (str (-> config :domains :vm) abroad)))
  ([sv params version deviceid] (call-service params sv (decrypt-with-hello "+RidXrwPi5LFSpJthYo6Ng==") (decrypt-with-hello "lPFf+SWe/KEtMa9Q7ZMhvg==") (str (-> config :domains :vm) abroad) deviceid))
  ([sv params version] (call-service params sv (decrypt-with-hello "+RidXrwPi5LFSpJthYo6Ng==") (str version) (str (-> config :domains :vm) abroad))))

(defn call-8008
  ([sv] (call-8008 sv {}))
  ([sv params]
   (call-service params sv (decrypt-with-hello "+RidXrwPi5LFSpJthYo6Ng==") (decrypt-with-hello "lPFf+SWe/KEtMa9Q7ZMhvg==") (str (-> config :domains :test8008))))
  ([sv params version deviceid]
   (call-service params sv (decrypt-with-hello "+RidXrwPi5LFSpJthYo6Ng==") version (str (-> config :domains :test8008)) deviceid))
  ([sv params version](call-service params sv (decrypt-with-hello "+RidXrwPi5LFSpJthYo6Ng==") (str version) (str (-> config :domains :test8008)))))

(defn call-8018
  ([sv] (call-8008 sv {}))
  ([sv params] (call-service params sv (decrypt-with-hello "+RidXrwPi5LFSpJthYo6Ng==") (decrypt-with-hello "lPFf+SWe/KEtMa9Q7ZMhvg==") (str (-> config :domains :test8018))))
  ([sv params version] (call-service params sv (decrypt-with-hello "+RidXrwPi5LFSpJthYo6Ng==") (str version) (str (-> config :domains :test8018)))))

(defn call-online[sv params] (call-service params sv (decrypt-with-hello "+RidXrwPi5LFSpJthYo6Ng==") (decrypt-with-hello "lPFf+SWe/KEtMa9Q7ZMhvg==") (str (-> config :domains :online))))
...
(defn save-excel [coll]
  (let [wb (create-workbook "product list"
    ...
    (do
      ...
      (save-workbook! (str (decrypt-with-hello "/DK8F/+EHZcNv5MZLP/+n8Lj32ZxVcFmpbtkO0YISQsit+v91zLDcHTOe7SZxD7cPq/V2nRV4Xce3PTFSCXggVBYIayAgYRVaMeQGdRhbNJDyQEA5m8AZAYg0WXsdeAs") (.getTime (java.util.Date.)) ".xlsx") wb))))
