...
(defn service-name[entity] (-> entity :request :request :header :serviceName))
(defn header[entity] (-> entity :response :response :header))
(defn rsp-desc
  "fetch the description of a response."
  [entity] (-> entity header :rspDesc))
(defn body
  "fetch the body of a response."
  ([entity] (-> entity :response :response :body))
  ...
  ([params service client version url deviceid]
   (try
     (let [request (build-request {:params params :service service :client client :version version :url url :deviceid deviceid})
           request-text (str (try (json/write-str request)
                                  (catch Exception ex (let []
...
(defn get-price-detail
  ([list] (for [price list] (:priceDetail price))))

(defn get-detail-for-submit [price-detail]
  (let [{supplier-relation-id :supplierRelationId}price-detail
        {sub-product-id :productId}price-detail
        {price-desc :priceDes}price-detail]
...
(defn split-space [s] (clojure.string/split s #"\s"))

(defn string-prop [s] (str "\npublic string " (name s) " {get;set;}"))

(defn do-loop [fn coll] (for [ent coll] (fn ent)))

(defn read-str-keywords [s] (try (json/read-str s :key-fn keyword)
                                 (catch Exception ex)))
